...
Std= math.sqrt((pi/4)*(1-(pi/4))/2000)     # defined standard deviation variable since std(sigma)=sqrt(p*(1-p)/N) where p is probability of lying random point in cicle

err=Std/ratio		       #defined variable err to calculte error estimation in value of pi since error in pi measurement is std/mean

print (err)		       #finally prints error  
